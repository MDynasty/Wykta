...
function findIngredientSection(text, preferredLang) {
  ...
  const metadataStopRe = /(?:生产批号|使用方法|生产日期|限期使用日期|(?:产\s*品\s*的\s*)?保\s*(?:质|鲜)\s*期|适用人群|使用注意|储存方法|产品批号|注意事项|生产企业|备案人|境内负责人|委托单位|致\s*敏\s*原(?:提示|警告|信息|说明)?)[：:]|(?:食品生产许可证编号|卫生许可证编号|生产许可证编号|(?:产\s*品\s*的\s*)?保\s*(?:质|鲜)\s*期|储存方法\s|保存方法|净\s*含\s*量|净\s*重|规\s*格|执\s*行\s*标\s*准(?:\s*号)?|产\s*品\s*标\s*准\s*(?:代\s*号|号)|标\s*准\s*代\s*号|有限公司|股份有限公司|股份公司|合伙企业|开封后请|开封后需|开封后立即|开封后应|见包装|喷码处|请置于阴凉|请存放于|请放置于|不受阳光直射|避免阳光直射|交叉口|本品在|本产品在|本\s*生\s*产\s*线|致\s*敏\s*原(?:提示|警告)?|过敏者请慎食|过敏者慎用|过敏者禁用|过敏体质者|过敏体质人群|含有过敏原|可能含有过敏原|如有不适请停用)|\b(?:Allergens?\b|Important\s+information\b|Nutritional?\s+information\b|Lot\s*(?:No?\.?\s*)?[#:\-–]|Batch\s*(?:No?\.?\s*)?[#:\-–]|(?:Manufactured|Distributed|Imported|Packaged|Produced|Bottled|Packed)\s+(?:by|for)\b|Made\s+in\b|Produced\s+in\b|Product\s+of\b|Fabriqu[eé]s?\s+(?:en|au|par)\b|Import[eé]\s+par\b|Distribu[eé]\s+par\b|Produit\s+(?:de|par)\b|Questions?\s+or\s+Comments?\b|For\s+more\s+information\b|Net\s*(?:Wt|Weight|Content)\b|Poids\s+net\b|Contenu\s+net\b|Netto(?:gewicht|inhalt)\b|Best\s+before\b|Use\s+by\b|Expiry(?:\s+date)?\b|Expiration(?:\s+date)?\b|MHD\b|SKU\b|EAN\b|Barcode\b|www\.[a-z]+|Laboratoires?\b|GmbH\b|Ltd\.?\b|Inc\.?\b|Corp\.?\b|S\.A\.S\.?\b|SAS\b|SARL\b|B\.V\.?\b|\d+\s*,?\s*(?:avenue|rue|boulevard|all[eé]e|strasse|street|place)\s+[A-Z])|©/i
