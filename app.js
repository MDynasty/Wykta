...
function isLikelyIngredientToken(token = ""){
  ...
  if (/[\u4e00-\u9fa5]/.test(normalized)) {
    if (/有限公司|股份有限公司|股份公司|合伙企业|食品生产许可证编号|卫生许可证编号|生产许可证编号|保\s*质\s*期|储存方法\s|保存方法|净\s*含\s*量|净\s*重|规\s*格|执\s*行\s*标\s*准(?:\s*号)?|产\s*品\s*标\s*准\s*(?:代\s*号|号)|标\s*准\s*代\s*号|开封后请|开封后需|开封后立即|开封后应|见包装|喷码处|请置于阴凉|请存放于|请放置于|不受阳光直射|避免阳光直射|交叉口|本品在|本产品在|本\s*生\s*产\s*线|致\s*敏\s*原/.test(normalized)) {
      return false
    }
